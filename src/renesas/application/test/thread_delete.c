...
static void thread1_cleanup(struct rt_thread *tid)
{
    ...
    //couldn't call printf in cleanup function, because printf included rt-thread IPC API calling,
    //As task stack has been released, it will have problem to use stack for deleted thread
...
static void thread2_entry(void* parameter)
{
	...
	rt_kprintf("thread2 begin\n");

    /* 线程2启动后先睡眠10个OS Tick */
    rt_thread_delay(RT_TICK_PER_SECOND);

    rt_kprintf("delete thread1\n");
    /*
    ...
    rt_kprintf("exit thread2 after %d ms\n",RT_TICK_PER_SECOND);
