...
void rt_hw_interrupt_init(void)
{
...
#if 0 // Use INTC driver IST Table
    ...
    arm_gic_dist_init(0, gic_dist_base, (int)gic_irq_start);
...
#else
    if (vector < INTC_VID_TOTAL)
    {
    	R_INTC_GetIntFunc(vector, &old_handler);
    	R_INTC_RegistIntFunc(vector, handler);
    }
